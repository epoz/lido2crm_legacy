 ...
 *
 */
public class DescriptiveMetaDataConverterMinimal {

	public Document descptiveMetaData;
	public Namespace lidoNamespace;
	public XPathFactory xpathFactory;

	public DescriptiveMetaDataConverterMinimal( Element descptiveMetaData, Namespace lidoNamespace){
	...
	public void addObjectClassificationWrap( Individual mainObject, OntModel model, String lidoRecID ){
		...
		for(Element objectWorkType:objectWorkTypeContainer){
			Object objectWorkTypeInd = LidoElementHandlerMinimal.getTypeClassification(objectWorkType, model, Namespace_Constants.BFM_NAMESPACE, "_101_", lidoNamespace, lidoRecID, String.valueOf(counter), CIDOC_CRM.E55_Type);

			if(objectWorkTypeInd instanceof  Individual )
				mainObject.addProperty(CIDOC_CRM.P2_has_type, (Individual) objectWorkTypeInd);

			if(objectWorkTypeInd instanceof  String )
				mainObject.addProperty(CIDOC_CRM.P2_has_type, model.getResource((String)objectWorkTypeInd));
		...
		for(Element classification:classificationContainer){
			Object classificationInd = LidoElementHandlerMinimal.getTypeClassification(classification, model, Namespace_Constants.BFM_NAMESPACE, "_102_", lidoNamespace, lidoRecID, String.valueOf(counter), CIDOC_CRM.E55_Type);

			if(classificationInd instanceof  Individual )
				mainObject.addProperty(CIDOC_CRM.P2_has_type, (Individual) classificationInd);

			if(classificationInd instanceof  String )
				mainObject.addProperty(CIDOC_CRM.P2_has_type, model.getResource((String)classificationInd));
	...
	public void addObjectIdentificationWrap(Individual mainObject, OntModel model, String lidoRecID){
		...
		for(Element title:titleContainer){
			...
			Individual titleResource = model.createIndividual( Namespace_Constants.BFM_NAMESPACE + lidoRecID + "_201_" + counter, CIDOC_CRM.E35_Title );
		...
		for(Element inscription:inscriptionContainer){
			...
			Individual inscriptionResource = model.createIndividual(Namespace_Constants.BFM_NAMESPACE + lidoRecID + "_202_" + counter, CIDOC_CRM.E34_Inscription);
		...
		for(Element repositoryName :legalBodyContainer){
			...
			Individual repositoryNameLegalBody = model.createIndividual(Namespace_Constants.BFM_NAMESPACE + lidoRecID + "_203_" + counter , CIDOC_CRM.E39_Actor);
		...
		if(place != null){
			mainObject.addProperty(CIDOC_CRM.P55_has_current_location, getPlace(place, model, lidoNamespace, lidoRecID, "_205"));
		}
		...
		for(Element displayStateEdition: displayStateContainer){
			...
			Individual displayStateResource = model.createIndividual(Namespace_Constants.BFM_NAMESPACE + lidoRecID + "_206_" + counter , CIDOC_CRM.E3_Condition_State);
			...
			for(Element sourceStateEdition : displayStateEdition.getChildren("sourceStateEdition", lidoNamespace)){
				if(sourceStateEdition.getChild("sourceStateEdition", lidoNamespace) != null)
		...
		for(Element objectDescription : objectDescriptionContainer){
			...
			Individual objectDescriptionResource = model.createIndividual(Namespace_Constants.BFM_NAMESPACE + lidoRecID + "_207_" + counter , CIDOC_CRM.E73_Information_Object);
			...
			mainObject.addProperty(CIDOC_CRM.P129i_is_subject_of, objectDescriptionResource);
			counter++;
		}
		...
		for(Element measurement : measurementContainer){
			...
			Individual measurementResource =  model.createIndividual(Namespace_Constants.BFM_NAMESPACE + lidoRecID + "_208_" + counter , CIDOC_CRM.E54_Dimension);
	...
	public void addEventWrap(Individual mainObject, OntModel model, String lidoRecID){
		...
		for(Element event:eventContainer){
			...
			mainObject.addProperty(CIDOC_CRM.P12i_was_present_at, eventResource);
			mainCounter++;
		}
	}
	...
	public void addObjectRelationWrap(Individual mainObject, OntModel model, String lidoRecID){
		...
		for(Element subject:subjectContainer){
			...
			Individual subjectInd = model.createIndividual(Namespace_Constants.BFM_NAMESPACE + lidoRecID + "_401_" + mainCounter, CIDOC_CRM.E73_Information_Object);
			...
			for ( Element subjectConcept : subject.getChildren("subjectConcept", lidoNamespace)){
				Individual subjectConceptInd = LidoElementHandlerMinimal.getType(subjectConcept, model, Namespace_Constants.BFM_NAMESPACE, "", lidoNamespace, 
						"_402_" + lidoRecID, String.valueOf(mainCounter) + "_" + String.valueOf(counter), CIDOC_CRM.E55_Type );
			...
			for (Element subjectActor : subject.getChildren("subjectActor", lidoNamespace)){
				if(subjectActor.getChild("actor", lidoNamespace) != null){
					Resource actorInd = getActor(subjectActor.getChild("actor", lidoNamespace), model, lidoNamespace, lidoRecID + "_403", String.valueOf( mainCounter ) );
			...
			for(Element subjectDate : subject.getChildren("subjectDate ",lidoNamespace)){
				...
				Individual timeSpan  = LidoElementHandlerMinimal.getTimeSpan( subjectDate,  model, Namespace_Constants.BFM_NAMESPACE, 
						"" ,  lidoNamespace,  lidoRecID,  "404_" + String.valueOf(mainCounter) + "_" + String.valueOf(counter));
			...
			for(Element subjectEvent : subject.getChildren("subjectEvent",lidoNamespace)){
				...
				if(subjectEvent.getChild("event", lidoNamespace) != null){
					Individual subjectEventInd = getEvent(model, lidoRecID + "_405", subjectEvent.getChild("event", lidoNamespace), mainCounter);
			...
			for ( Element subjectPlace : subject.getChildren("subjectPlace", lidoNamespace)){
				...
				if(subjectPlace.getChild("place", lidoNamespace).getChild("placeID", lidoNamespace) != null ){
					subjectInd.addProperty(CIDOC_CRM.P67_refers_to, getPlace(subjectPlace.getChild("place", lidoNamespace), model, lidoNamespace, lidoRecID, "_406") );
				}
			}
			...
			for ( Element subjectObject : subject.getChildren("subjectObject", lidoNamespace)){
				...
				if(subjectObject.getChild("object", lidoNamespace) != null)
			...
			mainObject.addProperty(CIDOC_CRM.P129i_is_subject_of, subjectInd);
			mainCounter++;
		}
	...
	private Individual getEvent(OntModel model, String lidoRecID, Element event, int eventCounter){
		...
		//Event
		Individual eventResource = model.createIndividual(Namespace_Constants.BFM_NAMESPACE + lidoRecID + "_301_" + eventCounter, CIDOC_CRM.E5_Event);

		Element eventType = event.getChild("eventType",lidoNamespace);
		if(eventType.getChild("conceptID", lidoNamespace) != null){
			String value = eventType.getChild("conceptID", lidoNamespace).getValue();

			if(isActivity(value))
				eventResource.addProperty(RDF.type, CIDOC_CRM.E7_Activity);
		}

		//1. EventID (0-n)
		int counter =1;
		for(Element eventID : event.getChildren("eventID",lidoNamespace)){
			Individual identifierEvent = model.createIndividual( Namespace_Constants.BFM_NAMESPACE + "_303_" + eventCounter + "_" + counter,  CIDOC_CRM.E42_Identifier );
			identifierEvent.addProperty(CIDOC_CRM.P3_has_note, eventID.getValue());	
			eventResource.addProperty(CIDOC_CRM.P1_is_identified_by, identifierEvent);	
			counter++;
		}
		...
		if(eventType != null){
			...
			Individual eventTypeInd = (Individual) LidoElementHandlerMinimal.getTypeClassification(eventType, model, 
					Namespace_Constants.BFM_NAMESPACE, "_304_" + String.valueOf(eventCounter), lidoNamespace, lidoRecID,"", CIDOC_CRM.E55_Type);
		...
		for(Element roleInEvent : event.getChildren("roleInEvent",lidoNamespace)){
			...
			Individual roleInEventInd = (Individual) LidoElementHandlerMinimal.getTypeClassification(roleInEvent, model, 
					Namespace_Constants.BFM_NAMESPACE, "_305_" , lidoNamespace, lidoRecID, String.valueOf(eventCounter) + "_" + String.valueOf(counter), CIDOC_CRM.E55_Type  );
		...
		for(Element eventName : eventNameContainer){	
			...
			Individual eventNameResource = model.createIndividual(Namespace_Constants.BFM_NAMESPACE + lidoRecID + "_306_" + eventCounter + "_" + counter, CIDOC_CRM.E41_Appellation);
		...
		String eventActorPath = "/lido:event/lido:eventActor";
		expr = xpathFactory.compile( eventActorPath, Filters.element(), null, lidoNamespace );
		List<Element> eventActorContainer = expr.evaluate(eventDoc);


		int actorCounter = 1;
		for(Element eventActor: eventActorContainer){

			//displayActorInRole (0-n) !? 
			//302

			//actorInRole(0-1)
			Element actorInRole = eventActor.getChild("actorInRole", lidoNamespace);

			if(actorInRole != null){

				Resource actor = getActor(actorInRole.getChild("actor", lidoNamespace) ,  model,  lidoNamespace,  lidoRecID, "_308_" + String.valueOf(eventCounter) + "_" + String.valueOf(actorCounter)  );

				//roleActor (0-n)
				if(actorInRole.getChildren("roleActor" , lidoNamespace) != null){

					Individual pc14_carried_out_by = model.createIndividual(Namespace_Constants.BFM_NAMESPACE + lidoRecID + "_309_" + eventCounter + "_" + actorCounter, CIDOC_CRM_PC.PC11_had_participant);
					pc14_carried_out_by.addProperty(CIDOC_CRM_PC.P01_has_domain, eventResource);
					pc14_carried_out_by.addProperty(CIDOC_CRM_PC.P02_has_range, actor);

					counter = 1;
					for(Element roleActor: actorInRole.getChildren("roleActor" , lidoNamespace)){

						Individual roleActorInd =  model.createIndividual(Namespace_Constants.BFM_NAMESPACE + lidoRecID + "_310_" + eventCounter + "_" + actorCounter + "_" + counter, CIDOC_CRM.E55_Type);

						//conceptID (0-n)
						for(Element conceptID  :roleActor.getChildren("conceptID ", lidoNamespace))
							roleActorInd.addProperty(CIDOC_CRM.P1_is_identified_by, conceptID .getValue());
		...
		for(Element culture: event.getChildren("culture",lidoNamespace)){
			...
			Individual culturInd = model.createIndividual(Namespace_Constants.BFM_NAMESPACE + lidoRecID + "_311_" + eventCounter + "_" + counter, CIDOC_CRM.E4_Period); //later specific type 
		...
		if( event.getChild("eventDate",lidoNamespace) != null ){
			...
			if( eventDate.getChild("date",lidoNamespace) != null){
				...
				Individual timeSpan  = LidoElementHandlerMinimal.getTimeSpan( eventDate.getChild("date",lidoNamespace),  model, Namespace_Constants.BFM_NAMESPACE, "",  lidoNamespace,  lidoRecID + "_312_", String.valueOf(eventCounter));
		...
		for(Element periodName: event.getChildren("periodName",lidoNamespace)){
			...
			Individual periodNameInd = model.createIndividual(Namespace_Constants.BFM_NAMESPACE + lidoRecID + "_313_" + eventCounter + "_" + counter, CIDOC_CRM.E4_Period); //later specific type 
		...
		for(Element eventPlace: event.getChildren("eventPlace",lidoNamespace)){
			...
			if( eventPlace.getChild("place", lidoNamespace) != null ){
				eventResource.addProperty( CIDOC_CRM.P7_took_place_at, getPlace(eventPlace.getChild("place", lidoNamespace), model, lidoNamespace, lidoRecID, "_314"));
			}
		}
		...
		for(Element eventMethod: event.getChildren("eventMethod",lidoNamespace)){
			Individual eventMethodInd = LidoElementHandlerMinimal.getType(eventMethod, model, Namespace_Constants.BFM_NAMESPACE, "", lidoNamespace, lidoRecID,
					"_315_" + String.valueOf(eventCounter) + "_" + String.valueOf(counter), CIDOC_CRM.E55_Type );
		...
		for(Element eventMaterialsTech: event.getChildren("eventMaterialsTech",lidoNamespace)){
			...
			if(eventMaterialsTech.getChild("materialsTech", lidoNamespace ) != null ){
				...
				for(Element termMaterialsTech : materialsTech.getChildren("termMaterialsTech", lidoNamespace)){
					...
					//E57 Material
					if(termMaterialsTechType != null)
						if(termMaterialsTechType.equals("http://terminology.lido-schema.org/lido00131")){
							eventMaterialsTechInd = LidoElementHandlerMinimal.getType(termMaterialsTech, model, Namespace_Constants.BFM_NAMESPACE, 
									"", lidoNamespace, lidoRecID, "_316_" + String.valueOf(eventCounter) + "_" + String.valueOf(counter), CIDOC_CRM.E57_Material);	
							...
							eventMaterialsTechInd = LidoElementHandlerMinimal.getType(termMaterialsTech, model, Namespace_Constants.BFM_NAMESPACE,
									"", lidoNamespace, lidoRecID, "_316_" + String.valueOf(eventCounter) + "_" + String.valueOf(counter), CIDOC_CRM.E55_Type);
		...
		for(Element relatedEventSet : event.getChildren("relatedEventSet ",lidoNamespace)){
			...
			if(relatedEventSet.getChild("relatedEventRelType",lidoNamespace) != null){
				...
				Individual relatedEventRelTypeInd = model.createIndividual(Namespace_Constants.BFM_NAMESPACE + "" + lidoRecID + "_317_" + eventCounter + "_" + counter, CIDOC_CRM.E55_Type); //later specific type 
		...
		for(Element eventDescriptionSet: event.getChildren("eventDescriptionSet", lidoNamespace)){
			...
			Individual eventDescription = model.createIndividual(Namespace_Constants.BFM_NAMESPACE + "" + lidoRecID + "_318_" +  + eventCounter + "_" + counter, CIDOC_CRM.E73_Information_Object); 
			...
			eventResource.addProperty(CIDOC_CRM.P129i_is_subject_of, eventDescription);
			counter++;
		}
	...
	private Individual getPlace(Element place , OntModel model, Namespace lidoNamespace, String lidoRecID, String id ){

		//place individual
		Individual placeInd = null;

		//if have individual ID
		for(Element palceID : place.getChildren("placeID", lidoNamespace)){		
			if( palceID.getAttributeValue("type", lidoNamespace ).equals("http://terminology.lido-schema.org/lido00100")){
				placeInd = model.createIndividual( Namespace_Constants.BFM_NAMESPACE + palceID.getValue().replace(" ", "") , CIDOC_CRM.E53_Place );
				break;
			}
		}
		...
		//Else generated ID
		if(placeInd == null)
			placeInd = model.createIndividual( Namespace_Constants.BFM_NAMESPACE + lidoRecID + id , CIDOC_CRM.E53_Place );
		...
		for(Element placeClassification: place.getChildren("placeClassification", lidoNamespace)){
			Individual placeClassificationInd = LidoElementHandlerMinimal.getType(placeClassification, model, Namespace_Constants.BFM_NAMESPACE, "placeClassification/", lidoNamespace, lidoRecID, String.valueOf(counter), CIDOC_CRM.E55_Type );
	...
	private Resource getActor( Element actor , OntModel model, Namespace lidoNamespace, String lidoRecID, String counter ){

		Resource actorInd = null;
		...
		for(Element actorID : actor.getChildren("actorID", lidoNamespace)){		
			if( actorID.getAttributeValue("type", lidoNamespace ).equals("http://terminology.lido-schema.org/lido00100")){
				return model.getResource(Namespace_Constants.BFM_NAMESPACE  + actorID.getAttributeValue("type", lidoNamespace ).replace(" ", "")); 
				//				actorInd = model.createIndividual( Namespace_Constants.BFM_NAMESPACE + actorID.getValue().replace(" ", "") , CIDOC_CRM.E39_Actor );
				//				break;
			}
		}

		if(actorInd == null)
			actorInd = model.createIndividual( Namespace_Constants.BFM_NAMESPACE + lidoRecID + "_" + counter, CIDOC_CRM.E39_Actor );
